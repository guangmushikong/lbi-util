...
public class GeoHash {
    ...
    final static int[] BITS={16,8,4,2,1};
    //初始化编码映射内容
    static {
        int i = 0;
        for (char c : digits)
    ...
    //对编码后的字符串解码
    public Envelope decodeEnvelope(String geohash) {
        double[] latInterval={-90.0,90.0};
        double[] lngInterval={-180.0,180.0};
        boolean isEven=true;
        for(int i=0;i<geohash.length();i++){
            int cd = lookup.get(geohash.charAt(i)).intValue();
            for(int mask:BITS){
                if(isEven){
                    if((cd & mask)!=0){
        ...
        return new Envelope(lngInterval[0],lngInterval[1],latInterval[0],latInterval[1]);
    }
